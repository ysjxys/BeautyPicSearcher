...
@property (nonatomic, strong) NSMutableArray        *cellRectArray;
@property (nonatomic, strong) NSMutableArray        *visibleCells;

@property (nonatomic, strong) NSMutableDictionary   *reuseDict;


@property (nonatomic, strong) NSMutableArray *columnHieghtArr;
...
@synthesize columnArr = _columnArr;
...
- (void)reloadData
{
    if (self.visibleCells && [self.visibleCells count] > 0) {
        for (int i = 0; i < [self.visibleCells count]; ++i) {
            ...
            int visibleCellCount = (int)[singleVisibleArray count];
...
- (BOOL)addObject:(id)obj Height:(CGFloat)height ColumnCount:(unsigned int)columnCount{
    if (!obj || height < 0) {
        return NO;
    }
    if (!self.columnArr) {
        if (columnCount > 0) {
            ...
            self.columnHieghtArr = [NSMutableArray array];
            for (int i = 0; i < columnCount; i++) {
                [tempArr addObject:[NSMutableArray array]];
                [self.columnHieghtArr addObject:[NSNumber numberWithFloat:0.0f]];
            }
            self.columnArr = tempArr;
        }else{
            return NO;
        }
    }
    int minColumnIndex = 0;
    for (int i = 1; i < self.columnHieghtArr.count; i++) {
        if ([self.columnHieghtArr[i] floatValue] < [self.columnHieghtArr[minColumnIndex] floatValue]) {
            minColumnIndex = i;
        }
    }
    CGFloat newHeight = height + [self.columnHieghtArr[minColumnIndex] floatValue];
    [self.columnHieghtArr replaceObjectAtIndex:minColumnIndex withObject:[NSNumber numberWithFloat:newHeight]];
    NSMutableArray *minColumnArr = self.columnArr[minColumnIndex];
    [minColumnArr addObject:obj];
    return YES;
}

- (NSMutableArray *)columnArray{
    return self.columnArr;
}

- (void)clearData{
    self.columnArr = nil;
    self.columnHieghtArr = nil;
    self.visibleCells = nil;
    self.cellRectArray = nil;
    self.reuseDict = nil;
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[WaterFlowViewCell class]]) {
        ...
        if (0 == [singleVisibleArray count]) {
            ...
            for (int j = 0; j < [singleRectArray count]; ++j) {
                ...
                if (![self canRemoveCellForRect:cellRect]) {
                    ...
                    cell.frame = cellRect;
                    cell.imageView.frame = CGRectMake(0, 0, cellRect.size.width, cellRect.size.height);
...
//                    NSLog(@"Cell Info : %@\n", cell);
            ...
            basicVisibleRow = (int)cell.indexPath.row;
        }
